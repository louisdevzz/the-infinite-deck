...
export const ConstructionPedestal: React.FC = () => {
    ...
    const [activeCard, setActiveCard] = useState<CyberCard>(MOCK_HOME_DATA.activeConstruction as CyberCard);
    ...
    return (
        <section className="col-span-12 lg:col-span-6 h-auto lg:h-full relative flex flex-col items-center justify-center perspective-container z-20 order-1 lg:order-2 py-8 lg:py-0">
            ...
            <div className="relative z-10 h-[45vh] md:h-[50vh] max-h-[460px] w-auto aspect-[2/3] tilt-center transition-all duration-700 hover:scale-105 group">
                {/* Floating Shadow */}
                ...
                <div className="w-full h-full bg-black/60 backdrop-blur-md rounded-2xl border border-primary/30 shadow-[0_0_40px_rgba(0,229,255,0.15)] flex flex-col p-5 relative overflow-hidden">
                    ...
                    <div className="flex-1 bg-black/80 rounded-xl overflow-hidden relative mb-5 border border-white/5 group-hover:border-primary/40 transition-colors shadow-inner">
                        ...
                        <img src={activeCard.image} alt={activeCard.name} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity duration-500 scale-105 group-hover:scale-110" />
                    ...
                    <div className="space-y-3 relative z-20">
                        <h2 className="text-3xl font-black italic text-white uppercase tracking-tighter drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] leading-none">{activeCard.name}</h2>
                        <div className="h-px w-full bg-gradient-to-r from-primary/50 to-transparent"></div>

                        <div className="flex justify-between items-end">
                            <div className="flex flex-col gap-1.5">
                                <span className="text-[9px] font-bold text-primary/60 uppercase tracking-[0.2em]">Molecule Density</span>
                                <div className="flex gap-1 h-2">
                                    {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
                                        <div key={i} className={`w-2 h-full rounded-[1px] ${i > 6 ? 'bg-white/10' : 'bg-primary shadow-[0_0_8px_rgba(0,229,255,0.6)]'}`}></div>
                                    ))}
                                </div>
                            </div>
                            <div className="text-right">
                                <span className="text-[9px] font-bold text-primary/60 uppercase tracking-[0.2em] block mb-0.5">Energy Output</span>
                                <span className="text-2xl font-mono font-bold text-white tracking-widest leading-none">{activeCard.stats.integrity}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-col items-center gap-6 mt-8 relative z-30">
                ...
                <div className="flex items-center gap-4 text-[10px] text-primary/40 font-mono tracking-widest uppercase">
                    ...
                    <span>::</span>
                    <span>AETHER LINK <span className="text-white font-bold">STABLE</span></span>
                </div>
            </div>
