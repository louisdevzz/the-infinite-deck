...
export const Backpack: React.FC<BackpackProps> = ({
  ...
  return (
    <aside className="w-full lg:w-[30%] lg:min-w-[250px] lg:max-w-[450px] bg-dark-charcoal/40 backdrop-blur-xl border-t lg:border-l border-white/10 flex flex-col overflow-hidden transition-all duration-300 order-3 lg:order-none h-[500px] lg:h-full shrink-0">
      <div className="p-8 border-b border-white/5 bg-black/20">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="bg-secondary/20 border border-secondary text-secondary text-[8px] px-3 py-1 font-bold uppercase tracking-widest animate-pulse">
            Event Active
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 card-grid-container bg-black/10">
        {items.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
            <span className="material-symbols-outlined text-5xl mb-4 text-white/20">
              inventory_2
            </span>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] mb-2">
              Storage Empty
            </p>
            <p className="text-[9px] font-mono text-white/40 max-w-[200px]">
              Acquire new nodes from the Market or Forge to expand your arsenal.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-5">
            ...
            {[...Array(items.length < 6 ? 6 - items.length : 0)].map((_, i) => (
              <div
                key={`empty-${i}`}
                className="aspect-[3/4] bg-white/5 border border-white/5 rounded-sm"
              ></div>
            ))}
          </div>
        )}
