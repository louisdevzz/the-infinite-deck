...
interface GameBoardProps {
  ...
  playerActiveCard?: CyberCard | null;
  opponentActiveCard?: boolean; // Just show back if true
  phase?: string;
}
  ...
  return (
    <section className="flex-1 flex flex-col relative overflow-hidden px-12 py-8 bg-gradient-to-b from-matrix-black to-background-dark">
      ...
      <div className="flex-1 flex flex-col items-center justify-start pt-4">
        <div className="text-[10px] font-black uppercase text-white/30 tracking-[0.5em] mb-4">
          Remote Uplink Node
        </div>
        <div className="w-48 h-64 holo-border rounded-sm bg-matrix-black/80 flex items-center justify-center relative group overflow-hidden">
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-matrix-black/40">
            ...
            {!opponentActiveCard && (
              <span className="absolute text-7xl font-black text-primary/40 glitch-text">
                ?
              </span>
            )}
          </div>
      ...
      <div className="flex items-center justify-center gap-12 my-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black tracking-[1em] text-primary/80 uppercase">
            Duel Field
          </span>
          <span className="material-symbols-outlined text-primary/60">
            grid_guides
      ...
      <div className="flex-1 flex flex-col items-center justify-end pb-12">
        <div className="w-48 h-64 border-2 border-dashed border-primary/40 rounded-sm bg-primary/[0.05] flex items-center justify-center relative shadow-[0_0_20px_rgba(0,229,255,0.1)] overflow-hidden">
          {playerActiveCard ? (
            <img
              src={playerActiveCard.img}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] font-bold text-primary/40 mt-2 uppercase tracking-widest">
                Active Slot
              </span>
            </div>
          )}
        </div>
        <div className="text-[10px] font-black uppercase text-primary tracking-[0.5em] mt-4">
          Local Root Core
        </div>
      </div>

      {/* Player Hand */}
      <Hand cards={hand} onCardClick={onCardClick} />
    </section>
  );
};
